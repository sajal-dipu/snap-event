...
export default function BookingsDashboardPage() {
  ...
  // Fetch all bookings for photographer
  const { data: bookings = [], isLoading, isError, refetch } = useQuery({
    queryKey: ["photographerBookings", user?.uid],
    queryFn: async () => {
      if (!user?.uid) return [];
      const res = await bookingService.list({ photographerId: user.uid }, 200);
      return res.data;
    },
    enabled: !!user?.uid
  });
